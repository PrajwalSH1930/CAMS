...
export default function StudentFee () {
  ...
  const loadUserFees = async () => {
    ...
    try {
      ...
      const result = await axios.get(`http://localhost:8080/fee/getFees/${studentId}`, config);
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      {fee ? (
        <div className="bg-white rounded-lg shadow-xl overflow-hidden">
          ...
          {fee.remaining_amount.toFixed(2) == 0 ? (
          ...
          <div className="p-6">
            {/* Fee General Info - Using Grid and Components */}
                    ...
                    fee.list.map((inst, index) => (
                      <tr key={inst.installmentId} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                        ...
                        <td className="py-3 px-4 text-center">
                          {inst.paid !== "YES" ? (
                            <Link
                              className="px-4 py-2 text-sm font-semibold rounded-md text-blue-700 bg-blue-100 hover:bg-blue-200 transition-colors"
                              to={`/student/payInstallment/${fee.studentId}/${inst.installmentId}`}
                            >
                              💳 Pay
                            </Link>
                          ) : (
            ...
            <div className="mt-6 flex flex-wrap gap-4">
              {fee.installment !== "Yes" && fee.remaining_amount !== 0 && (
                <Link
                  className="px-6 py-3 text-lg font-bold rounded-lg bg-green-500 text-white hover:bg-green-600 transition-colors"
                  to={`/student/payFees/${fee.studentId}`}
                  state={{ amount: (fee.total_amount - (fee.concession_perc / 100 * fee.total_amount)) }}
                >
                  💵 Pay Full Fees
                </Link>
              )}
              {!fee.installment && fee.remaining_amount !== 0 && (
                <Link
                  className="px-6 py-3 text-lg font-bold rounded-lg bg-yellow-500 text-white hover:bg-yellow-600 transition-colors"
                  to={`/student/updateFees/${fee.studentId}`}
                >
                  📅 Set Up Installment
                </Link>
              )}
        ...
        <div className="p-6 text-center text-red-500 text-lg">Please complete the registration and apply for admission!</div>
      )}
    </div>
  );
};